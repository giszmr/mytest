...
int non_block_connect()
{
    ...
    servaddr.sin_addr.s_addr = htonl(0xc0a80305);
    servaddr.sin_port   = htons(8000);

    printf("zhumengri1\n");
    socketHandle = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    ioctl(socketHandle, FIONBIO, &nonblockMode);
#if 0
    ...
    if(FD_ISSET(socketHandle, &writefds))
        printf("in writefds\n");
    else
        printf("not in writefds\n");
